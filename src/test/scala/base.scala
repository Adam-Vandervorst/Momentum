...
class BaseTest extends FunSuite:
  ...
  test("source mapTo") {
    ...
    val (src, f) = callback[Boolean]
    val true_src: SetAdaptor[Boolean, Unit] = src.mapS(_.contramapTo({unevaluated = false; true}))
    src(if _ then a = a + 1)
    f(true)
  ...
  test("source map") {
    ...
    var b = 0
    val (src, f) = callback[Boolean]
    val neg_src: SetAdaptor[Boolean, Unit] = src.mapS(_.contramap(!_))
    src(if _ then a = a + 1)
  ...
  test("source deplete map") {
    ...
    val src = deplete(xs)
    val double_src: SetAdaptor[Int, Unit] = src.mapS(_.contramap(_*2))
    src(ys.addOne)
  ...
  test("source foldLeft") {
    ...
    var b = 0
    val (src, f) = callback[Boolean]
    val pos_count: SetAdaptor[Int, Unit] = src.mapS(_.scan(0)((t, b) => t + (if b then 1 else 0)))
    val neg_count: SetAdaptor[Int, Unit] = src.mapS(_.scan(0)((t, b) => t + (if b then 0 else 1)))
    neg_count(a = _)
    f(true)
    assert(a == 0)
    pos_count(b = _)
